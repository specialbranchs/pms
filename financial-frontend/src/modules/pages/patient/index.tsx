...
const PatientHomeScreen = () => {
    ...
    const singleAppointMent = () => {
        ...
        api.report.getSingleAppointment$(user?.id, date)
            .pipe(
    ...
    const CountApporvedAppointMent = () => {
        const date = new Date().toISOString().slice(0, 10)
        api.report.CountInQueue(1, date)
            .pipe(
                doOnSubscribe(() => setLoading(true)),
                finalize(() => setLoading(false))
            )
    ...
    return (
        <>
            <Grid item xs={12}>
                <Card sx={{ display: 'flex', justifyContent: 'space-between', padding: 5, backgroundColor: "#e0f7ed" }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ flex: '1 0 auto' }}>
                            ...
                            <Typography sx={{
                                ...
                                {app ? "Please be patience.. we will look" : "Please, Confirm your appointment.."}
                            </Typography>
                        </CardContent>
            ...
            <Grid item xs={12} marginTop={4} textAlign={'center'}>
                <Card sx={{
                    display: 'flex', flexDirection: {
                        ...
                        lg: 'row',
                        xl: 'row'
                    }, justifyContent: 'space-around', alignItems: 'center', padding: 5
                }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <CardContent sx={{ flex: '1 0 auto' }}>
                            ...
                            {app && <Typography variant="subtitle1"
                                sx={{
                                    color: status(app.status).color, ...fontFamily,
                                    fontSize: {
                                        xs: 12,
                                        sm: 16,
                                        md: 18,
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ flex: '1 0 auto' }}>
                            ...
                            {app && <Typography variant="subtitle1" sx={{
                                backgroundColor: "#e0f7ed", padding: 1, ...fontFamily,
